...
func (o *OdooConn) ResPartnerCustomerUnlink() {
	...
	stmt := `select distinct trim(leading '0' from kunnr) kunnr,cname from sapdata.customers c order by cname`
	type Partner struct {
		Ref   string `db:"kunnr"`
	...
	partners := o.SearchRead(umdl, oarg{}, 0, 0, []string{"ref"})
	ids := []int{}
...
func (o *OdooConn) ResPartnerCustomer(custtype string) {
	// customer
	mdl := "res_partner"
	umdl := strings.Replace(mdl, "_", ".", -1)
	...
	stmt := `
	select distinct
	cname,trim(leading '0' from kunnr) kunnr,trim(leading '0' from parent) parent
	,street,city,state,country,zip,salesregion,ctype,is_company
	...
	tagIDs := o.SearchRead("res.partner.category", oarg{}, 0, 0, []string{"name"})
	tids := map[string]int{}
...
// 	stmt2 := stmt + `where c.kunnr not in (select distinct trim(leading '0' from billto) from odoo.artg_customer_hierarchy)`
// 	// fmt.Println(stmt2)
